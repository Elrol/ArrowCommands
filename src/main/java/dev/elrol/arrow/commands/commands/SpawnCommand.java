...
public class SpawnCommand extends _CommandBase {
    @Override
    public void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess, CommandManager.RegistrationEnvironment environment) {
        dispatcher.register(CommandManager.literal("spawn")
                .requires((source)->{
                    if(source.isExecutedByPlayer() && source.getPlayer() != null) {
                        return PermUtils.hasPerm(source.getPlayer(), "arrow.command", "spawn").asBoolean();
                    }
                    return true;
                })
    ...
    private int noArgs(CommandContext<ServerCommandSource> context) {
        ...
        if(player != null) {
            ...
            commandData.teleportToSpawn(player);
            context.getSource().sendMessage(ModTranslations.msg("spawn"));
        } else {
